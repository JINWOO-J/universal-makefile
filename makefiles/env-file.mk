...
ifneq (,$(wildcard .runner.env))
    include .runner.env
    export
    $(info [INFO] .runner.env 파일 로드됨 (중앙서버 전파))
endif
...
prepare-consul-runtime-env: ## 🔧 Consul + 로컬 환경 변수 병합하여 $(RESOLVED_ENV_FILE) 생성
	...
	@{ \
	  TMP=$$(mktemp .env.XXXXXX); \
	  : "NOTE: env_manager export(--use-consul)는 Consul + 로컬(.env.common/.env.{env}/.runner.env/.env.local/.build-info)까지 이미 병합된 최종 값만 출력함"; \
	  ...
	  \
	  if [ -f ".env.local" ] && grep -q '^DEPLOY_IMAGE=' .env.local 2>/dev/null; then \
	    LOCAL_DEPLOY_IMAGE=$$(grep '^DEPLOY_IMAGE=' .env.local | cut -d= -f2); \
	  ...
	  if [ "$$USE_GIT_IMAGE" = "true" ] && [ -d "$(SOURCE_DIR)" ] && cd "$(SOURCE_DIR)" >/dev/null 2>&1 && git rev-parse --git-dir >/dev/null 2>&1; then \
	    ...
	    mv "$$TMP2" "$$TMP"; \
	    printf "$(YELLOW)계산된 이미지 태그 (Git 기반):$(NC)\n"; \
	    printf "  IMAGE_TAG       : %s\n" "$$IMAGE_TAG"; \
	    printf "  DEPLOY_IMAGE    : %s\n" "$$IMAGE_TAG"; \
	  else \
	    if [ "$$USE_GIT_IMAGE" = "false" ]; then \
	      printf "$(YELLOW)사용자 정의 DEPLOY_IMAGE 유지:$(NC)\n"; \
	      printf "  DEPLOY_IMAGE    : %s (소스: %s)\n" "$$EXISTING_DEPLOY_IMAGE" "$$DEPLOY_SOURCE"; \
	    else \
	      echo "$(YELLOW)[WARNING]$(NC) SOURCE_DIR or git not ready; skipping DEPLOY_IMAGE calculation"; \
	    fi; \
	  fi; \
