...
install_release() {
  ...
  # 1) Determine desired version/ref
  local desired=""
  ...
  # 5) Determine top directory name
  local ROOT_DIR_NAME
...
main() {
  ...
  case "$cmd" in
    install)
      ...
      && umc_create_project_config "${MAKEFILE_DIR}" "${FORCE_INSTALL}" \
      && umc_update_gitignore \
